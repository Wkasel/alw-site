...
************************************************************************************************************/	
	...
	function JSDragDropTree()
	{
		...
		this.imageFolder = '/source/images/admin/tree/';
		...
		this.filePathRenameItem = '/admin/edit_tree/';
		this.filePathDeleteItem = '/admin/delete_tree/';
	...
	JSDragDropTree.prototype = {
		...
		*/
		...
		{
			var message = 'Удалить объект ' + obj2.innerHTML;
			if(this.hasSubNodes(obj2.parentNode)) message = message + ' и его внутренние элементы';
			if(confirm(message)){
		...
		,
		__createNewTextBox : function(obj)
		{
			var textBox = document.createElement('INPUT');
			textBox.className = 'folderTreeTextBox';
			textBox.value = obj.innerHTML;
		...
		addItem : function(obj,obj2) {
			var object_id = obj2.parentNode.id.replace(/[^0-9]/gi,'');
			document.getElementById('parent_id').value = object_id;
			document.getElementById('object_type').value = obj2.parentNode.getAttribute('object_type');
			setVisible('add_object');
		}
		,
		editObject :  function(obj, obj2) {
			var object_id = obj2.parentNode.id.replace(/[^0-9]/gi,'');
			var object_type = obj2.parentNode.getAttribute('object_type');
			var real_id = obj2.parentNode.getAttribute('real_id');
			window.location = '/admin/index/'+object_type+'/edit_item/'+real_id+'/';
		},
		initTree : function()
		{
			JSTreeObj = this;
			...
			if(this.deleteAllowed || this.renameAllowed){
				try{
					...
					if(this.renameAllowed)menuModel.addItem(2,'Редактировать','','',false,'JSTreeObj.editObject');
					if(this.renameAllowed)menuModel.addItem(3,'Изменить название','','',false,'JSTreeObj.renameItem');
					if(this.renameAllowed)menuModel.addItem(4,'Добавить объект','','',false,'JSTreeObj.addItem');
					menuModel.init();	
					
					var menuModelRenameOnly = new DHTMLGoodies_menuModel();
					if(this.renameAllowed) {
						menuModelRenameOnly.addItem(3,'Редактировать','','',false,'JSTreeObj.renameItem');
						menuModelRenameOnly.addItem(4,'Добавить объект','','',false,'JSTreeObj.addItem');
					}
