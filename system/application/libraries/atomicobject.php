...
class AtomicObject {
    ...
    $this->nodel = isset($this->structure['nodel']) ? $this->structure['nodel'] : false;
    $this->noadd = isset($this->structure['noadd']) ? $this->structure['noadd'] : false;
    $this->nofoot = isset($this->structure['nofoot']) ? $this->structure['nofoot'] : false;
    $this->save = isset($this->structure['save']) ? $this->structure['save'] : false;
    $this->input_field = isset($this->structure['input_field']) ? $this->structure['input_field'] : array();
		...
		} elseif (strpos($_SERVER['REQUEST_URI'], 'update_nodes') !== false) {
    ...
    $this->layout['nodel'] = $this->nodel;
    $this->layout['noadd'] = $this->noadd;
    $this->layout['nofoot'] = $this->nofoot;
    $this->layout['save'] = $this->save;
    $this->layout['input_field'] = $this->input_field;
	...
	function update_nodes() {
		if (is_array($_POST['product_id'])) {
			foreach ($_POST['product_id'] as $key => $value)
			 $this->CI->db->query('UPDATE products set available_inventory = ' . $_POST['available_inventory'][$key] . ' WHERE products_id = ' . $value);
		}
		redirect($_SERVER['HTTP_REFERER']);
	}
