...
<table cellpadding="0" cellspacing="0" class="inline">
  ...
  {if $save}
	...
	<form action="/{$my_mode}/index/{$table}/delete_nodes/" id="form_delete" method="POST">
  {if !$nofoot}
	<tfoot>
		<tr>
  ...
  {/if}
	...
	<tr style="background-color: #{if $smarty.foreach.rows_list.iteration%2}FFF{else}f8f8f8{/if};">
		...
		{foreach from=$row item=row_field key=column name=f}	
			{if $column != 'table_id'}
				<td>{if in_array($column,$input_field)}<input type="hidden" name="product_id[]" value="{$row->table_id}"><input name="available_inventory[]" type="text" value="{$row_field}" style="width:100%">{else}{$row_field}{/if}</td>
			{/if}
		{/foreach}	
	</tr>
	{/foreach}
