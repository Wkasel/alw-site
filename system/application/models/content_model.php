...
class Content_Model extends Model
{
	...
	function mobileImagesOnMain() {
		$query = $this->db->query('SELECT * FROM mobile_main_images');
		$arr = array();
		foreach ($query->result() as $node) {
			$arr[] = $node; 
		}
	...
	function auth($login, $password, $remember=false) {
		...
		$query = $this->db->query('SELECT office_id, reps_login, token FROM office 
		...
		if ($query->num_rows() == 0) {
			...
			$this->db->query('UPDATE office SET token="'.md5(strval(time()).$secretkey).'" WHERE reps_login = '.$this->db->escape($login).' AND reps_password = "'.md5($password).'"');
			$query = $this->db->query('SELECT office_id, reps_login, token FROM office 
		WHERE reps_login = '.$this->db->escape($login).' AND reps_password = "'.md5($password).'"');
			return $query->row();
		}
	...
	// WTF is this $user = BS? -- fix this crap.
	...
	function getQuickShipNewObjects() {
		$arr = array();
		return $arr;
	}
